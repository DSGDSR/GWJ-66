...
func _run_splash_screen(splash: String) -> void:
	...
	# Show splash
	splash_tween.tween_property(splash_image, 'modulate:a', 1, 0.75).set_ease(Tween.EASE_IN).set_delay(0.75)
	splash_tween.tween_property(splash_image, 'modulate:a', 0, 2).set_ease(Tween.EASE_OUT).set_delay(1.5)
